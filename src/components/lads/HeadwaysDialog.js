...
export default function HeadwaysDialog(props) {
  ...
  const SetHeadway = () => {
    ...
    if (headway) {
      setItem({
        ...item, 
        headways: [...item.headways.map(hdw => {
  ...
  return (
    <div>
      ...
      >
        <DialogTitle>
          Edit Headway and Service Time – LAD {item.code}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={3}>
            ...
            <Grid item xs={3}>
              <TextField 
                ...
                onChange={(e) => setItem({ ...item, serviceTime: { start: e.target.value, end: item.serviceTime.end }})}
              />
            </Grid>
            <Grid item xs={3}>
              <TextField 
                ...
                onChange={(e) => setItem({ ...item, serviceTime: { end: e.target.value, start: item.serviceTime.start }})}
              />
            </Grid>
            <Grid item xs={2}>
            </Grid>
            ...
            <TextField 
                ...
                onChange={(e) => setLocal({ ...local, fromTime: e.target.value })}
              />
            </Grid>
            <Grid item xs={3}>
            <TextField 
                ...
                onKeyUp={(e) => { if (e.key == 'Enter') { SetHeadway(); } }}
                    ...
                    <div key={hdw.fromTime}>
                      ...
                      >
                        <ListItemButton onClick={() => setLocal(hdw)}>
                          <ListItemText>
                            From time: <strong>{hdw.fromTime}</strong> Headway value: <strong>{hdw.value}</strong>
                          </ListItemText>
                      ...
                      <Divider sx={{ width: '90%', margin: 'auto' }} component="li" />
                    </div>
                  ))}
                </List>
              </Box>
